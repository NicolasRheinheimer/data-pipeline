...
def transformar_vendas():
    ...
    # Cria as colunas calculadas de lucro
...
def transformar_contas_receber():
    ...
    # renomeia 'nome' para 'nome_cliente' logo no merge do financeiro também
    ...
    # colunas sejam tratadas como Data pelo Pandas
    df_consolidado['data_vencimento'] = pd.to_datetime(df_consolidado['data_vencimento'])
    df_consolidado['data_pagamento'] = pd.to_datetime(df_consolidado['data_pagamento'])
    ...
    # qual data usar no cálculo: data de pagamento (se houver) ou data de hoje
    ...
    # diferença bruta de dias tratando como uma Série limpa do Pandas
